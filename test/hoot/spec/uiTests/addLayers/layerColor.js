...
describe(" Active Color for New Layers ", function() {
  ...
  // it("should select a layer from the table", done => {
  //   d3.select(
  //     '#add-ref-table g[data-name="BostonSubsetRoadBuilding_FromOsm"]'
  //   ).dispatch("click");
  //   d3.select("#add-ref-table").dispatch("click");
  //   d3.select(
  //     "#reference  div  fieldset  div.hoot-form-field.action-container button"
  //   ).dispatch("click");
  //   d3.select(
  //     "#bar  div.map-controls  div.map-control.zoombuttons  button.zoom-in"
  //   ).dispatch("click");
  //   d3.select(
  //     "#bar  div.map-controls  div.map-control.zoombuttons  button.zoom-in"
  //   ).dispatch("click");
...
describe(" Can change basemap ", () => {
  it(" Basemap can be changed", done => {
    d3.select(
      "#bar  div.map-controls div.map-control.background-control"
    ).dispatch("click");
    setTimeout(() => {
      done();
    }, 5000);
