...
export function uiMapMetadata(data, context) {
    ...
    function load(d) {
        ...
        if (d.tags && d.tags.stats) {
            ...
            if (stats['Waterway Count']) {
                ...
                layerfeatures[ConfLayerName].waterways = stats['Waterway Count'][2];
    ...
    mapMetadata.button = function(selection) {
        ...
        button.enter().insert('button', '[id^="palette"]')
            .attr('tabindex', -1)
            .attr('class', 'map-metadata-button map-button keyline-left inline')
