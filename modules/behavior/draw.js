...
export function behaviorDraw(context) {
    ...
    function click() {
        ...
        if (target && target.type === 'node' && Hoot.layers.loadedLayers === 1 ||  target && target.type === 'node' && _activeLayer.id === Number(target.id.split('_')[1])) {   // Snap to a node
                dispatch.call('clickNode', this, target, d);
                return;

        } else if (target && target.type === 'way' && Hoot.layers.loadedLayers === 1 || target && target.type === 'way' && _activeLayer.id === Number(target.id.split('_')[1])) {   // Snap to a way
