iD.ui.Tools = function( context ) {
    ...
    function tools() {
        ...
        var items = [];
        items.push( {
            title: 'Measurement Tools', icon: 'line', group: 'measure', items: [
                {
                    ...
                    type: 'line',
                    icon: 'line',
                    mode: iD.modes.MeasureAddLine( context )
                },
                {
                    title: 'Measure Area',
                    tooltip: 'Shortcut: 7',
                    ...
                    type: 'area',
                    icon: 'area',
                    mode: iD.modes.MeasureAddArea( context )
                },
                {
                    title: 'Measure Help',
                    tooltip: '',
        ...
        if ( !context.hoot().control.conflicts.isConflictReviewExist() ) {
            items.push( {
                title: 'Clip Tools', icon: 'clip', group: 'clip', items: [
                    {
                        title: 'Clip Dataset',
                        tooltip: 'Shortcut: 8',
                        group: 'clip',
        ...
        var toolsItem = d3.selectAll( '.tools-menu' )
            ...
            .on( 'mouseenter', function( item ) {
                ...
                subTools.append( 'span' ).text( function( item ) {
                    return item.title;
        ...
        toolsItem.append( 'span' ).text( function( item ) {
            return item.title;
        } );
        ...
        d3.select( '.tools-menu' )
            .style( 'left', function() {
                return Math.ceil( d3.select( 'button.tools' ).node().getBoundingClientRect().left ) + 'px' || '0px';
            } )
            .style( 'top', '120px' )
            .style( 'display', 'block' );
