...
Hoot.control.conflicts.map.featurehighlighter = function( context ) {
    ...
    _instance.highlightLayer = function( ritem, raitem, panTo ) {
        ...
        if ( feature ) {
            ...
            if ( context.hoot().checkForValidCoordinates( feature.extent( context.graph() )[ 0 ] ) &&
                context.hoot().checkForValidCoordinates( feature.extent( context.graph() )[ 1 ] ) ) {
                panToId = feature.id;
            }
        ...
        if ( againstFeature ) {
            ...
            if ( !panToId && context.hoot().checkForValidCoordinates( againstFeature.extent( context.graph() )[ 0 ] ) &&
                context.hoot().checkForValidCoordinates( againstFeature.extent( context.graph() )[ 1 ] ) ) {
    ...
    var _performHighlight = function( fid, ftype, offType, on, offFid ) {
        ...
        if ( context.hasEntity( fid ) === undefined || context.hasEntity( offFid ) === undefined ) {
            return;
        }
