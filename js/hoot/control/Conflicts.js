...
Hoot.control.conflicts = function( context, sidebar ) {
    ...
    _instance.startReview = function( data ) {
        ...
        var keybinding = d3.keybinding( 'conflicts' )
            .on( da[ 0 ].cmd, function() {
                d3.event.preventDefault();
                _callHotkeyAction( da[ 0 ] );
            } )
            .on( da[ 1 ].cmd, function() {
                d3.event.preventDefault();
                _callHotkeyAction( da[ 1 ] );
            } )
            .on( da[ 2 ].cmd, function() {
                d3.event.preventDefault();
                _callHotkeyAction( da[ 2 ] );
            } )
            .on( da[ 3 ].cmd, function() {
                d3.event.preventDefault();
                _callHotkeyAction( da[ 3 ] );
            } )
            .on( da[ 4 ].cmd, function() {
                d3.event.preventDefault();
                _callHotkeyAction( da[ 4 ] );
            } )
            .on( da[ 5 ].cmd, function() {
                d3.event.preventDefault();
                _callHotkeyAction( da[ 5 ] );
            } );
        ...
        _instance.actions.traversereview.initialize( { 'nextid': 'next', 'previd': 'previous', 'mapid': _mapid } );

        // get the first item
        _instance.actions.traversereview.jumpTo( 'forward' );

        //Register listener for review layer cleanupimport
    ...
    _instance.vischeck = function() {
        ...
        var vis    = _.filter( layers, function( d ) {
            return d.vis;
        } );
        if ( vis.length > 1 ) {
