...
Hoot.control.export = function(context, sidebar) {
	...
	exp.activate = function(layer, translations) {
		...
		// The layer here does not have the "canExportToOsmApiDb" property at
		// this point like it
		// does in ExportDataset, so we need to get the map tags for the layer
		...
		Hoot.model
				.REST(
						...
						function(tags) {
							...
							// This timestamp tag is what the server uses to
							// determine if a layer can be exported to
							// an OSM API db.
							...
							if (layer.canExportToOsmApiDb === true) {
								exportFormatList.push({
									'DESCRIPTION' : 'OSM API Database'
								});
							}
							...
							save = sidebar.append('form').classed(
									'round space-bottom1', true);
							save
									.append('a')
									.classed(
											'button dark animate strong block _icon big plus pad2x pad1y js-toggle active',
											true).attr('href', '#')
							...
							save
									.append('fieldset')
									.classed(
											'pad1 keyline-left keyline-right keyline-bottom round-bottom',
											true)
									...
									.html(
											function(d) {
												if (d.checkbox) {
													var retval = '<label class="pad1x pad0y round-top '
															+ d.checkbox
															+ '" style="opacity: 1;">';
													retval += '<input type="checkbox" class="reset checkbox" style="opacity: 1;">'
															+ d.label
															+ '</label>';
													return retval;
												} else {
												...
												if (field.inputtype === 'text') {
													if (field.transcombo) {
														...
														if (defTrans === undefined) {
															return field.transcombo[0].DESCRIPTION;
														} else {
									...
									.select(
											function(a) {
												if (a.checkbox) {
													d3
															.selectAll(
																	'input.reset.appendFGDBTemplate')
															.remove();
													d3
															.select(
																	'.cboxAppendFGDBTemplate')
															.select('input')
															.property(
																	'checked',
																	false);
							...
							var actions = save.select('fieldset').append('div')
							...
							actions
									...
									.attr('value', 'Exit')
									.classed(
											'fill-darken0 button round pad0y pad2x small strong',
											true).attr('border-radius', '4px')
									.on('click', function() {
										d3.event.stopPropagation();
										d3.event.preventDefault();
		...
		function checkForTemplate() {
			...
			if (exportType === '') {
				exportType = d3.select('.fileExportFileType').attr(
						'placeholder');
			}
			if (transType === '') {
				transType = d3.select('.fileExportTranslation').attr(
						'placeholder');
			}
			...
			var selTrans = _.find(transCombo, {
			...
			if (hidden) {
				d3.select('.cboxAppendFGDBTemplate').select('input').property(
						'checked', false);
			}
		}
		...
		function toggleForm(context) {
			...
			d3.select(context).classed('active', text);
		}
		return save;
	};
