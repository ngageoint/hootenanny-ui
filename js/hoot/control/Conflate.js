...
Hoot.control.conflate = function(sidebar, context) {
	...
	_instance.advancedoptions = Hoot.control.conflate.advancedoptions(context,
			_instance);//, sidebar);
	...
	_instance.activate = function(data) {
		...
		context.hoot().model.folders.listFolders(context.hoot().model.folders
				.getAvailFolders());
	...
	var _populateDefaultCombo = function(a) {
		var combo = d3.combobox().data(_.map(a.combobox, function(n) {
			return {
				value : n,
				title : n
			};
		...
		comboCnt.style('width', '100%').call(combo);
	...
	var _populateFolderListCombo = function(a) {
		var comboPathName = d3.combobox().data(
				_.map(a.combobox.data, function(n) {
					return {
						value : n.folderPath,
						title : n.folderPath
					};
		...
		context.hoot().model.layers
				.setLayerLinks(function() {
					...
					context.hoot().model.folders
							.listFolders(context.hoot().model.folders
									.getAvailFolders());
					var folderList = _.map(context.hoot().model.folders
							.getAvailFolders(), _.clone);

					//build the path array for lyrA
					var lyrApath = [];
					lyrA = _.find(context.hoot().model.layers.getAvailLayers(),
							{
								'name' : lyrA
							});
					if (lyrA) {
					...
					lyrB = _.find(context.hoot().model.layers.getAvailLayers(),
							{
								'name' : lyrB
							});
					if (lyrB) {
		...
		if (commonId !== 0) {
			var match = _.find(a.combobox.data, {
				id : commonId
			});
			if (match) {
				if (match) {
		...
		d3.select(this).style('width', '100%').call(comboPathName);
	};
	...
	var _populateReferenceCombo = function(a) {
		...
		var combo = d3.combobox().data(_.map(a.combobox.data, function(n) {
			return {
				value : n,
				title : n
			};
		...
		if (a.type === 'newfoldername' || a.type === 'saveAs') {
			d3.select(this).on(
					...
					function() {
						...
						var resp = context.hoot().checkForUnallowedChar(
								this.value);
						if (resp !== true) {
							d3.select(this).classed('invalidName', true).attr(
									'title', resp);
						} else {
							d3.select(this).classed('invalidName', false).attr(
									'title', null);
						}
					});
		}
	...
	var _advanceOptionsLnkHandler = function() {
		...
		function displayHandler(ai) {
			...
			if (ai.hidden.length > 0) {
				d3.select(selAI.node().parentNode).style('display', ai.hidden);
			}
		}
		...
		if (!_instance.confAdvOptsDlg) {
			...
			if (_instance.confAdvOptsDlg.classed('hidden')) {
				...
				_instance.confAdvOptsDlg.classed(
						'fill-darken3 pin-top pin-left pin-bottom pin-right',
						true);
				_instance.advancedoptions.advOpsFormEvent(true);
			} else {
				if (window
						.confirm('Exit will remove any previously selected advanced options. Are you sure you want to exit?')) {
					_removeAdvancedOptionsDlg();
				}
			}
		}
	...
	var _isFieldsValuesValid = function() {
		...
		if (!_.isEmpty(_.filter(_.map(_.pluck(context.hoot().model.layers
				.getAvailLayers(), 'name'), function(l) {
			...
			iD.ui
					.Alert(
							'A layer already exists with this name.'
									+ ' Please remove the current layer or select a new name for this layer.',
							'warning', new Error().stack);
			return false;
		}

		var resp = context.hoot().checkForUnallowedChar(
				_container.selectAll('.saveAs').value());
		if (resp !== true) {
			iD.ui.Alert(resp, 'warning', new Error().stack);
			return false;
		}

		resp = context.hoot().checkForUnallowedChar(
				_container.selectAll('.newfoldername').value());
		if (resp !== true) {
			iD.ui.Alert(resp, 'warning', new Error().stack);
			return false;
		}

		var parId = context.hoot().model.folders.getfolderIdByName(_container
				.selectAll('.pathname').value()) || 0;
		resp = context.hoot().model.folders.duplicateFolderCheck({
			name : _container.selectAll('.newfoldername').value(),
			parentId : parId
		});
		if (resp !== true) {
			iD.ui.Alert(resp, 'warning', new Error().stack);
	...
	var _conflateBtnClickHandler = function() {
		...
		if (!_instance.confAdvOptionsSelectedVal) {
			...
			_instance.confAdvOptionsFields = _instance.advancedoptions.fieldsretriever
					.getDefaultFields();
			_instance.confAdvOptionsSelectedVal = _instance.advancedoptions.selectionretriever
					.getSelectedValues(null, _instance.confAdvOptionsFields);
		}

		if (d3.selectAll('.reset.ConfType').value() === 'Advanced Conflation') {
			_instance.lastAdvFields = _.map(_instance.confAdvOptionsFields,
					_.clone);
			_instance.lastAdvValues = _.map(
					_instance.confAdvOptionsSelectedVal, _.clone);
			_instance.lastAdvDlg = [];
			_.each(d3.select('#CustomConflationForm').selectAll('form')
					.selectAll('input')[0], function(ai) {
				var selAI = d3.select('#' + ai.id);
				_instance.lastAdvDlg.push({
					id : ai.id,
					type : ai.type,
					checked : ai.checked,
					value : ai.value,
					disabled : selAI.property('disabled'),
					hidden : d3.select(selAI.node().parentNode)
							.style('display')
				});
			});
		}
	...
	var _submitLayer = function(a) {
		...
		self
				...
				.html(
						'<div class="pad1 inline _loading"><span></span></div>'
				...
				.on(
						...
						function() {
							...
							d3.event.preventDefault();
							if (window
									.confirm('Are you sure you want to cancel conflation job?')) {

								d3
										.select('#hootconflatecontrol')
										.html(
												'<div class="pad1 inline _loading"><span></span></div>'
	...
	var _restorePrevValues = function() {
		_instance.lastAdvFields = _instance.lastAdvFields ? _instance.lastAdvFields
				: null;
		_instance.lastAdvValues = _instance.lastAdvValues ? _instance.lastAdvValues
				: null;
		_instance.lastAdvDlg = _instance.lastAdvDlg ? _instance.lastAdvDlg
				: null;
	};
	...
	var _removeLastAdvanceOptDlg = function() {
		...
		if (_instance.confAdvValsDlg) {
			_instance.confAdvValsDlg.remove();
		}
	...
	var _toggleForm = function(context) {
		if (/active/g.test(context.className)) {
			sidebar.selectAll('.js-toggle').classed('active', false);
		} else {
			sidebar.selectAll('.js-toggle').classed('active', false);
			d3.select(context).classed('active', true);
		}
	};
	...
	var _getFormMeta = function() {
		...
		var folderList = _.map(context.hoot().model.folders.getAvailFolders(),
				_.clone);
		...
		return [
				...
				},
				{
					...
					combobox : {
						'data' : folderList,
						'command' : _populateFolderListCombo
					},
					readonly : 'readonly'
				},
				{
				...
				},
				{
					...
					combobox : [ 'true', 'false' ],
					onchange : function() {
						var selVal = d3.selectAll('.reset.isCollectStats')
								.value();
						return selVal;
					},
					readonly : 'readonly'
				},
				{
					...
					combobox : [ 'true', 'false' ],
					onchange : function() {
						var selVal = d3.selectAll('.reset.isGenerateReport')
								.value();
						return selVal;
					},
					readonly : 'readonly',
	...
	var _getSaveAsOutputName = function(data) {
		...
		if (data[0].name.indexOf('OSM_API_DB') > -1) {
			...
			newName = data[0].name.replace('OSM_API_DB_', '') + '_'
					+ data[1].name;
		} else {
			...
			if (!newName.found) {
				...
				newName = 'Merged_' + newName.substring + '_'
	...
	var _createHeaderToggleBtn = function(data) {
		_container = sidebar.append('form').classed(
				'hootConflate round space-bottom1', true);

		_container
				.data(data)
				.append('a')
				.classed(
						'button dark animate strong block _icon big conflate conflation pad2x pad1y js-toggle',
						true).attr('href', '#').text('Conflate').on(
						'click',
						function() {
							d3.event.stopPropagation();
							d3.event.preventDefault();

							if (d3.select('#sidebar2').selectAll('input')
									.property('disabled')) {
								return;
							}
	...
	var _createLayerRefThumbnails = function(data) {
		var layerRef = _container.append('fieldset').classed(
				'hidden pad1 keyline-all', true).append('div').classed(
				'conflate-heading center contain space-bottom1 pad0x', true);
		...
		layerRef
				...
				.attr(
						'class',
						function(d) {
							return 'thumb round _icon data contain dark inline pad1 fill-'
									+ d.color;
						}).attr('id', function(d) {
	...
	var _createFieldSets = function(formMeta) {
		...
		fieldset
				...
				.classed(
						'form-field fill-white small keyline-all round space-bottom1',
						true)
				.html(
						function(field) {
							if (field.type === 'ConfType') {
								...
								return '<label class="pad1x pad0y strong fill-light round-top keyline-bottom">'
										+ field.label + '</label>';
							}
						}).append('input').attr('type', 'text').attr('value',
						function(a) {
	...
	var _createActionContainer = function() {
		var actions = _container.select('fieldset').append('div').classed(
				'form-field pill col12', true);
	...
	var _createCancelBtn = function(actions) {
		actions
				...
				.attr('type', 'submit')
				.attr('value', 'Cancel')
				.classed('fill-darken0 button round pad0y pad2x small strong',
						true)
				...
				.on(
						...
						function() {
							...
							if (window
									.confirm('Cancel will remove any previously selected advanced options. Are you sure you want to cancel?')) {
								_removeAdvancedOptionsDlg();
								sidebar.selectAll('.js-toggle').classed(
										'active', false);
							}
	...
	var _createConflationSubmitBtn = function(actions) {
		actions
				...
				.attr('id', 'conflate2')
				.classed(
						'fill-dark button round pad0y pad2x dark small strong margin0',
						true).attr('border-radius', '4px').on('click',
						_conflateBtnClickHandler);
	};
				...
				.on(
						...
						function() {
							...
							if (d3.selectAll('.reset.ConfType').value() === _instance.lastAdvSettingsText) {
								...
								if (_instance.confAdvOptionsSelectedVal == null) {
									...
									_instance.confAdvOptionsSelectedVal = _instance.advancedoptions.selectionretriever
											.getSelectedValues(
													null,
													_instance.confAdvOptionsFields);
								}
							}
							...
							iD.ui
									.Alert(
											JSON
													.stringify(_instance.confAdvOptionsSelectedVal),
											'notice', null);
						});
