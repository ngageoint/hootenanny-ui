...
Hoot.control.import = function(context, selection) {
	...
	ETL.render = function(colors, isPrimary) {
		context.map().on(
				'maxImportZoomChanged',
				function() {
					var imp = d3.selectAll('.hootImport')[0];
					if (imp.length) {
						for ( var i = 0; i < imp.length; i++) {
							var n = d3
									.select(d3.selectAll('.hootImport')[0][i])
									.select('.button').node();
							if (n) {
								hideForm(n);
							}
		...
		var palette = _.filter(context.hoot().palette(), function(d) {
			return d.name !== 'green';
		});
		var d_form = [ {
			label : 'Layers',
			...
			combobox : _.map(context.hoot().model.layers.getAvailLayers(),
					function(n) {
						return n.name;
					}),
		...
		var sidebar = selection.selectAll('forms').data(colors).enter();
		var _form = null;
		if (isPrimary) {
		...
		_form
				.classed(
						'hootImport round space-bottom1 importableLayer fill-white strong',
						true)
				.on('submit', function() {
					submitLayer(this);
				})
				.append('a')
				.classed(
						'button dark animate strong block _icon big plus pad2x pad1y js-toggle',
						true).attr('href', '#').text(function() {
					var pnode = d3.select(d3.select(this).node().parentNode);
					if (isPrimary || pnode.node().nextSibling) {
						return 'Add Reference Dataset';
					} else {
						return 'Add Secondary Dataset';
					}
				}).on('click', function() {
					toggleForm(this);
				});
		...
		fieldset
				.classed(
						'pad1 keyline-left keyline-right keyline-bottom round-bottom hidden',
						true).selectAll('.form-field').data(d_form).enter()
				.append('div').classed('overflow', true).style({
					'height' : '150px',
					'margin' : '0 0 15px',
					'resize' : 'vertical'
		...
		var recentLayersDiv = fieldset.append('div').classed(
				'form-field fill-white small keyline-all round space-bottom1',
				true);
		recentLayersDiv.append('label').classed(
				'pad1x pad0y strong fill-light round-top keyline-bottom', true)
		...
		recentLayersDiv
				...
				.select(
						function() {
							...
							var comboData = context.hoot().model.layers
									.getRecentlyUsedLayers();
							var combo = d3.combobox().data(
									_.map(comboData, function(n) {
										return {
											value : n,
											title : n
										};
		...
		fieldset
				...
				.attr(
						'class',
						function(p) {
							var active = (d3.select(this.parentNode).datum() === p.name) ? ' active _icon check'
									: '';
							var osm = '';
							if (p.name === 'osm') {
								osm = ' _osm';
				...
				.on(
						...
						function() {
							...
							d3.select(this.parentNode).selectAll('a').classed(
									'active _icon check', false);
							d3.select(this).classed('active _icon check', true);
							d3.select(this.parentNode).datum(
									d3.select(this).datum().name);
						});
		fieldset.append('div').classed('form-field col12', true)
				.append('input').attr('type', 'submit').attr('value',
						'Add Layer').classed(
						'fill-dark pad0y pad2x dark small strong round', true)
				.attr('border-radius', '4px');

		// Add Layer click handler
		var submitLayer = function(a) {
			if (context.hoot().model.layers.isLayerLoading() === true) {
				iD.ui.Alert('Please wait utill loading first layer is done!',
						'notice');
				return;
			}
			...
			var color = self.select('.palette .active').attr('data-color');
			...
			try {
				...
				if (self.select('.sel').empty()) {
					...
					if (self.selectAll('.usedLayersInput').value() === '') {
						iD.ui.Alert(
								'Please select a dataset to add to the map!',
								'warning', new Error().stack);
						return;
					} else {
						lyrname = self.selectAll('.usedLayersInput').value();
						lyrid = context.hoot().model.layers
								.getmapIdByName(lyrname);
						if (lyrid == null) {
					...
					var gNode = d3
							.select(self.select('.sel').node().parentNode);
					if (!gNode.selectAll('title').empty()) {
						lyrname = gNode.select('title').text();
					} else {
						lyrname = gNode.select('.dnameTxt').text();
					}
					lyrid = gNode.select('.dnameTxt').attr('lyr-id');
				}
			} catch (e) {
				iD.ui.Alert('There was an error adding this layer to the map!',
						'warning', new Error().stack);
				return;
			}
			if (!lyrname || !lyrid) {
			...
			if (context.hoot().model.layers.getLayers()[lyrname]) {
				iD.ui
						.Alert(
								'A layer with this name has already been added to the map!',
								'warning', new Error().stack);
				return;
			}
			var key = {
			...
			context.hoot().model.layers
					.addLayer(
							...
							function(res) {
								context.hoot().model.layers
										.setRecentlyUsedLayers(key.name);
								//update combo boxes
								var comboData = context.hoot().model.layers
										.getRecentlyUsedLayers();
								var combo = d3.combobox().data(
								...
								d3.selectAll('.usedLayersInput').each(
										function() {
								...
								if (res === 'showprogress') {
									self
											.attr(
													...
													function() {
														...
														return 'round space-bottom1 loadingLayer '
																+ color;
													}).select('a').remove();
									...
									self
											.append('div')
											.classed(
													'contain keyline-all round controller',
													true)
											.html(
								...
								context.hoot().control.view
										.on(
												'layerColor.background',
												function(lyrname, color, mapid) {
								...
								context.hoot().control.view.on(
										'layerRemove.background', function(
												layerName, isPrimary, mapId) {
											context.background().removeSource(
													getNodeMapnikSource({
														name : layerName,
														id : mapId
													}));
							...
							function getNodeMapnikSource(d) {
								var source = {
									...
									template : window.location.protocol
											...
											+ iD.data.hootConfig.nodeMapnikServerPort
											+ '/?z={zoom}&x={x}&y={y}&color='
											+ encodeURIComponent(context.hoot()
													.palette(d.color))
											+ '&name=' + d.name,
									scaleExtent : [ 0, 18 ],
									overlay : true,
	...
	var resetForm = function(self) {
		self.select('.controller').remove();
		self
				.insert('a', 'fieldset')
				.attr('href', '#')
				.text('Add dataset')
				.classed(
						'button dark animate strong block js-toggle _icon big plus pad2x pad1y',
						true)
				.on(
						'click',
						function() {
