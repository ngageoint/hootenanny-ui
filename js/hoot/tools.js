...
Hoot.tools = function(context) {
	var loadingLayer = {}, loadedLayers = {}, activeConflateLayer = {}, ETL = context
			.hoot().control.import, view = context.hoot().control.view, conflate = context
			.hoot().control.conflate, conflicts = context.hoot().control.conflicts, exportLayer = context
			.hoot().control.export, hoot = context.hoot(), colors = [ 'violet',
			'orange' ], exporting = false;
		...
		if (add) {
			...
			if (confLayers) {
				_.each(confLayers, function(d) {
					...
					d3.select('.hootView.layer_' + loadedLayers[d].mapId)
							.remove();
				});
			}
		}
	...
	function preConflation(a, layerName, advOpts) {
		...
		var refLayer = '1';
		var oRefLayerId = hoot.model.layers.getmapIdByName(a.select(
		...
		data.CONFLATION_TYPE = _confType[a.select('.ConfType').value()]
				|| a.select('.ConfType').value();
	...
	function postConflation(item, a) {
		...
		if (pathname === '') {
			pathname = a.select('.reset.PathName').attr('placeholder');
		}
		if (pathname === 'root') {
			pathname = '';
		}
		...
		context.hoot().model.folders.addFolder(folderData, function(folderId) {
			...
			if (a.select('.saveAs').value()) {
				link.mapid = _.pluck(_.filter(context.hoot().model.layers
						.getAvailLayers(), function(f) {
					return f.name === a.select('.saveAs').value();
				}), 'id')[0] || 0;
			}
			if (link.mapid === 0) {
				return;
	...
	function renderMergedLayer(layerName, mapid) {
		...
		Hoot.model.REST('getMapTags', {
			...
			loadedLayers[layerName] = _.extend(loadedLayers[layerName],
					loadingLayer);
			view.render(loadingLayer);
			loadingLayer = {};
			conflicts.activate(loadedLayers[layerName]);
			context.hoot().mode('edit');
			context.hoot().model.conflicts.beginReview(activeConflateLayer,
					function(d) {
	...
	conflicts.on('exportData', function() {
		...
		Hoot.model.REST('getMapSize', mapid, function(sizeInfo) {
			...
			if (totalSize > expThreshold) {
				...
				var res = window.confirm('Export data size is greater than '
						+ thresholdInMb
	...
	conflicts.on('addData', function() {
		...
		d3
				.select(
						'[data-layer="' + activeConflateLayer.mapId.toString()
								+ '"]').remove();
	...
	conflate
			.on(
					'merge',
					function(a, layerName, advOptions) {

						var layers = inputLayers();
						// get map id
						var input1_id = context.hoot().model.layers
								.getmapIdByName(layers[0]);
						var input2_id = context.hoot().model.layers
								.getmapIdByName(layers[1]);
						// and then check size
						//getMapSize
						Hoot.model
								.REST(
										...
										function(sizeInfo) {
											...
											if (totalSize > confThreshold) {
												var thresholdInMb = Math
														.floor((1 * confThreshold) / 1000000);
												if (!window
														.confirm('Conflation data size is greater than '
																+ thresholdInMb
											...
											var data = preConflation(a,
													layerName, advOptions);
											...
											if (data.AUTO_TUNNING === 'true') {
												...
												context
														...
														.autotune(
																...
																function(res1) {
																	var result1 = JSON
																			.parse(res1.statusDetail);

																	data.INPUT1_ESTIMATE = ''
																			+ result1.EstimatedSize;
																	var data2 = {};
																	...
																	context
																			...
																			.autotune(
																					...
																					function(
																							res2) {
																						var result2 = JSON
																								.parse(res2.statusDetail);
																						data.INPUT2_ESTIMATE = ''
																								+ result2.EstimatedSize;
																						context
																								.hoot().model.conflate
																								.conflate(
																										conflationExecType,
	...
	context
			...
			.on(
					'layerAdded',
					function(layerName) {
						var params = context.hoot().model.layers
								.getLayers(layerName);
						if (!params) {
							return;
						}
																			...
																			if (tags.reviewtype === 'hgisvalidation') {
																				...
																				if (r === true) {
																					...
																					context
																							.hoot()
																							.replaceColor(
																									loadedLayers[layerName].id,
																									selColor);
																					context
																							.hoot().model.layers
																							.changeLayerCntrlBtnColor(
																									loadedLayers[layerName].id,
																									selColor);
																					activeConflateLayer = loadingLayer;
																					loadedLayers[layerName] = _
																							.extend(
																					...
																					loadingLayer = {};
																					conflicts
																							.activate(loadedLayers[layerName]);
																					context
																							.hoot()
																							.mode(
																									'edit');
																					...
																					if (tags) {
																						...
																						var input1unloaded = input1Name ? null
																								: input1;
																						var input2unloaded = input2Name ? null
																								: input2;
																						...
																						if ((input1 && input1Name)
																													...
																													context
																															...
																															.addLayer(
																																	...
																																	function(
																																		...
																																		context
																																				.hoot().model.layers
																																				.setLayerInvisibleById(input2);

																																		if (d === undefined) {
																																			context
																																					.hoot().model.conflicts
																																					.beginReview(
																																							activeConflateLayer,
																																							function(
																																									d) {
																							...
																							if (input1
																								...
																								iD.ui
																										.Alert(
																												'Could not determine input layer 2. It will not be loaded.',
																												'warning',
																												new Error().stack);
																							} else if (input2
																									&& input2Name) {
																								...
																								iD.ui
																										.Alert(
																												'Could not determine input layer 1. It will not be loaded.',
																												'warning',
																												new Error().stack);
																							} else {
																								iD.ui
																										.Alert(
																												'Could not determine input layers 1 or 2. Neither will not be loaded.',
																												'warning',
																												new Error().stack);
																							}
																					...
																					if (params.hideinsidebar !== undefined
																							&& params.hideinsidebar === 'true') {
																						doRenderView = false;
																					}
	...
	exportLayer
			.on(
					'saveLayer',
					function(cont, data) {
						var exportType = cont.select(
								'.reset.fileExportFileType.combobox-input')
								.value();
						exporting = true;
						var spinner = cont.append('span').attr('class',
								'spinner-hoot').call(iD.ui.Spinner(context));
						context.hoot().model.export
								.exportData(
										cont,
										data,
										function(status) {
											if (status === 'failed') {
												iD.ui
														.Alert(
																'Export has failed or partially failed. For detail please see Manage->Log.',
																'error',
																new Error().stack);
											}

											if (exportType
													&& exportType === 'Web Feature Service (WFS)') {
												var tblContainer = d3
														.select('#wfsdatasettable');
												context.hoot().view.utilities.wfsdataset
														.populateWFSDatasets(tblContainer);
											}
