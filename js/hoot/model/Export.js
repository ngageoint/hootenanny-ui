...
Hoot.model.export = function(context) {
	...
	model_export.exportData = function(container, data, callback) {
		...
		outputname = container.select('#fileExportOutputName').value()
				|| container.select('#fileExportOutputName')
						.attr('placeholder');
		selectedInput = data.name || outputname;

		selExportTypeDesc = container.select('#fileExportFileType').value()
				|| container.select('#fileExportFileType').attr('placeholder');
		var _expType = {
		...
		var selectedTranslation = 'translations/'
				+ iD.data.hootConfig.defaultScript;
		...
		if (!selectedInput || !selectedOutType) {
			iD.ui.Alert('Please enter valid values.', 'error',
					new Error().stack);
			return;
		}
		...
		var appendTemplate = '';
		try {
			appendTemplate = container.select('.cboxAppendFGDBTemplate')
		...
		d3.json('/hoot-services/job/export/execute').header('Content-Type',
				'text/plain').post(
		...
		if (result.status !== 'running') {
			...
			if (result.status !== 'failed') {
				...
				if (selectedOutType === 'wfs') {
					...
					context.hoot().control.utilities.wfsdataset
							.wfsDetailPopup(param);
				} else if (selectedOutType === 'osm_api_db') {
					//OSM API db export writes directly to an osm api database and involves no file 
					//download for export.
					var summaryStartIndex = result.statusDetail
							.indexOf('Changeset(s)');
					var summary = result.statusDetail
							.substring(summaryStartIndex);
					...
					var sUrl = '/hoot-services/job/export/' + result.jobId
							+ '?' + outNameParam + '&removecache=true';
					var link = document.createElement('a');
					link.href = sUrl;
					if (link.download !== undefined) {
						//Set HTML5 download attribute. This will prevent file from opening if supported.
						var fileName = sUrl.substring(
								sUrl.lastIndexOf('/') + 1, sUrl.length);
